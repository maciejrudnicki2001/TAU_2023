...
def test_check_item_in_cart(x):
    ...
    if ("https://www.saucedemo.com/cart.html" == driver.current_url and x == 0):
        if(driver.find_element(By.ID, "item_4_title_link").is_displayed()):
            print("Test passed item is in cart")
        else:
            print("Test failed item is not in cart")
